...
public class ImageStreamUrl : MonoBehaviour {
  ...
  [SerializeField] private float noImageTimeout = 2f;
  private float currentNoImageTimer = 0f;
  ...
  private void Update() {
    currentNoImageTimer += Time.deltaTime;

    if (currentNoImageTimer >= noImageTimeout) {
      ClearRenderTexture();
      currentNoImageTimer = 0;
    }
